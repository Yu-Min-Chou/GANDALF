...
def loadData(pathToDatasetFolder, oneHot=False):
    ...
    # pathToFile = os.path.join(pathToDatasetFolder, "CINIC-10/")
    ...
    for i in range(len(imgNamesTrain)):
        img = imageio.imread(os.path.join(imgDirsTrain[i], imgNamesTrain[i]))
        if len(img.shape) == 2:
            XTrain[i, :, :, 2] = XTrain[i, :, :, 1] = XTrain[i, :, :, 0] = img
        else:
            XTrain[i] = img
    ...
    #     img = imageio.imread(os.path.join(imgDirsVal[i], imgNamesVal[i]))
    #     if len(img.shape) == 2:
    #         XVal[i, :, :, 2] = XVal[i, :, :, 1] = XVal[i, :, :, 0] = img
    #     else:
    #         XVal[i] = img
    #     YVal[i] = labelDict[os.path.basename(imgDirsVal[i])]
    print("Loading test dataset")
    for i in range(len(imgNamesTest)):
        img = imageio.imread(os.path.join(imgDirsTest[i], imgNamesTest[i]))
        if len(img.shape) == 2:
            XTest[i, :, :, 2] = XTest[i, :, :, 1] = XTest[i, :, :, 0] = img
        else:
            XTest[i] = img
